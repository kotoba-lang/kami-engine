...
pub fn generate_hair(params: &HairParams) -> MeshPart {
    ...
    for s in 0..n_strands {
        let (start_theta, start_phi) = strand_origin(params.preset, s, n_strands, params.part_position);
        ...
        for seg in 0..n_segments {
            ...
            let w = w0 * (1.0 - t * 0.8);
            let x = sx + hash_f32(s as u32, seg + 100) * 0.004 * t + t * 0.008 * (curl + t * 1.5).sin();
            let y = sy - t * length;
            let z = sz + hash_f32(s as u32, seg + 200) * 0.004 * t + t * 0.005 * start_theta.sin();

            let right = Vec3::new((start_theta + PI / 2.0).cos(), 0.0, (start_theta + PI / 2.0).sin()) * w;
            ...
            vertices.push(Vertex { position: pos_l, normal, uv: [0.0, t] });
            vertices.push(Vertex { position: pos_r, normal, uv: [1.0, t] });
...
fn preset_config(preset: HairPreset) -> (u32, u32, f32) {
    match preset {
        ...
        HairPreset::ShortStraight | HairPreset::ShortWavy | HairPreset::ShortCurly => (150, 8, 0.07),
        HairPreset::Bob => (180, 10, 0.10),
        HairPreset::MediumStraight | HairPreset::MediumWavy | HairPreset::MediumLayered => (200, 10, 0.15),
