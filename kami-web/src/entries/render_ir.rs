...
//! look is hardcoded here. It is **stateful + idempotent**: the first call sets up
//! wgpu (once, stored thread-local); every call re-uploads the instances and draws a
//! frame. So the CLJS host can call it each requestAnimationFrame to get a *live*
//! game (sim → render-IR → draw), with no per-frame re-init.
//!
//! IR: `{globals:{sky{horizon,sun-dir,sun}}, instances:[{pos,color,size,yaw}]}`.
...
fn draw(st: &State, ir: &serde_json::Value) -> Result<(), JsValue> {
    let g = &ir["globals"];
    let sky_h = arr3(&g["sky"], "horizon");
    let sun_dir = arr3(&g["sky"], "sun-dir");
    let sun_col = arr3(&g["sky"], "sun");
    // optional camera follow: globals {eye:[3], target:[3]}; else overview of centroid
