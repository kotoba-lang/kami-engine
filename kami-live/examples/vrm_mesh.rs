...
fn humanoid_mesh() -> (Vec<Vertex>, Vec<u16>) {
    ...
    cylinder(
        &mut v,
        &mut idx,
        Vec3::new(-0.17, 1.4, 0.0),
        Vec3::new(-0.52, 1.05, 0.0),
        0.06,
        0.05,
        3,
    ); // L arm
...
fn palette(pose: &kami_live::DancePose) -> Palette {
    ...
    let root_motion = Mat4::from_translation(Vec3::new(
    ...
    for i in 0..7 {
        ...
        world[i] = if PARENT[i] < 0 {
            local
        } else {
            world[PARENT[i] as usize] * local
        };
    }
    let mut joints = [[[0.0f32; 4]; 4]; MAX_JOINTS];
    for i in 0..MAX_JOINTS {
        let jm = if i < 7 {
            world[i] * rest[i].inverse()
        } else {
            Mat4::IDENTITY
        };
...
async fn run() {
    ...
    let pbuf = device.create_buffer(&wgpu::BufferDescriptor {
        label: None,
        size: std::mem::size_of::<Palette>() as u64,
        usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
        mapped_at_creation: false,
    });
    ...
    let vbl = wgpu::VertexBufferLayout {
        ...
        attributes: &wgpu::vertex_attr_array![0=>Float32x3, 1=>Float32x3, 2=>Uint32x4, 3=>Float32x4],
    };
    ...
    let vp = Mat4::perspective_rh(0.9, w as f32 / h as f32, 0.1, 100.0)
        * Mat4::look_at_rh(eye, target, Vec3::Y);

    let mut scene = DanceScene::from_edn(SCENE).expect("scene");
    scene.show.start();
    for _ in 0..(61.0 * 60.0) as i32 {
        scene.frame(1.0 / 60.0);
    } // advance to the Chorus (wota: arms up)
