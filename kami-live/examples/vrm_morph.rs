//! Seed-san (real VRM): + EXPRESSION MORPH (blink/aa/happy via ExpressionManager) +
...
async fn run() {
    ...
    for frame in 0..32 {
        ...
        if frame%8==0 { image::save_buffer(format!("seedmorph_{frame:02}.png"),&px,w,h,image::ExtendedColorType::Rgba8).unwrap(); }
        gif.push(image::Frame::from_parts(image::RgbaImage::from_raw(w,h,px).unwrap(),0,0,image::Delay::from_numer_denom_ms(70,1)));
    }
    let fl=std::fs::File::create("seed_morph.gif").unwrap();
    ...
    println!("wrote seed_morph.gif + seedtex_*.png — Seed-san: + expression morph (blink/aa/happy)");
}
