//! Seed-san (real VRM): + SPRING BONES (hair/gear jiggle via SpringSimulator) +
...
async fn run() {
    ...
    let mut morph_prims: Vec<(usize, usize, usize, Vec<Vec<[f32;3]>>)> = Vec::new();
    for node in nodes.iter() {
        ...
        for pi in 0..mesh.primitives.len() {
            ...
            if !prim.targets.is_empty() {
                let mut deltas: Vec<Vec<[f32;3]>> = Vec::new();
                for tgt in &prim.targets {
                    let d = tgt.get("POSITION").and_then(|v| v.as_u64())
    ...
    let mgr = kami_vrm::ExpressionManager::new(&doc.expressions);
    let mut spring = kami_vrm::spring::SpringSimulator::new(&doc);
    println!("spring: {} chains, {} joints, {} colliders", spring.chain_count(), spring.joint_count(), spring.collider_count());
    println!("expressions: {:?}", doc.expressions.iter().map(|e| e.name.clone()).collect::<Vec<_>>());
    println!("morph prims: {}", morph_prims.len());
    ...
    for frame in 0..32 {
        ...
        // show-driven expression weights → ExpressionManager → vertex morph (CPU).
        ...
        for e in &doc.expressions {
            ...
            let w = if ln.contains("blink") && !ln.contains("left") && !ln.contains("right") { blink }
                ...
                else if ln.contains("happy") || ln.contains("joy") || ln.contains("fun") || ln.contains("smile") { happy }
                else { 0.0 };
            if w > 0.0 { ew.insert(e.name.clone(), w); }
        }
        let resolved = mgr.resolve(&ew);
        let mut mv = verts.clone();
        for (gstart, count, mi, deltas) in &morph_prims {
        ...
        queue.write_buffer(&vbuf, 0, bytemuck::cast_slice(&mv));
        ...
        // SPRING BONES: simulate hair/gear, apply the resulting local rotations, re-run FK.
        let mut sout: Vec<(usize, [f32; 4])> = Vec::new();
        spring.step(1.0 / 60.0, |n| world.get(n).copied(), &mut sout);
        for (node, q) in &sout {
            let t = nodes[*node].translation.map(Vec3::from).unwrap_or(Vec3::ZERO);
            let sc = nodes[*node].scale.map(Vec3::from).unwrap_or(Vec3::ONE);
        ...
        if frame%8==0 { image::save_buffer(format!("seedspring_{frame:02}.png"),&px,w,h,image::ExtendedColorType::Rgba8).unwrap(); }
        gif.push(image::Frame::from_parts(image::RgbaImage::from_raw(w,h,px).unwrap(),0,0,image::Delay::from_numer_denom_ms(70,1)));
    }
    let fl=std::fs::File::create("seed_spring.gif").unwrap();
    ...
    println!("wrote seed_spring.gif + seedtex_*.png — Seed-san: + spring bones (hair/gear jiggle)");
}
