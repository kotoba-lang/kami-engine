//! Seed-san (real VRM) TEXTURED + GPU-skinned + danced by the clj/edn show.
...
async fn run() {
    ...
    let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor{label:None,source:wgpu::ShaderSource::Wgsl(r#"
        ...
        @fragment fn fs(i: VO) -> @location(0) vec4<f32> {
          ...
          return vec4<f32>(base.rgb * (0.45 + 0.65*d), 1.0);
        }
    "#.into())});
    ...
    for frame in 0..32 {
        ...
        if frame%8==0 { image::save_buffer(format!("seedtex_{frame:02}.png"),&px,w,h,image::ExtendedColorType::Rgba8).unwrap(); }
        gif.push(image::Frame::from_parts(image::RgbaImage::from_raw(w,h,px).unwrap(),0,0,image::Delay::from_numer_denom_ms(70,1)));
    }
    let fl=std::fs::File::create("seed_textured.gif").unwrap();
    ...
    println!("wrote seed_textured.gif + seedtex_*.png — Seed-san textured + skinned + dancing");
}
