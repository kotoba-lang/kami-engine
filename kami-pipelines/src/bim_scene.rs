//! `BimSceneAdapter` — renders a `kami_bim::StoreyScene` projection
//! plus supports mouse-projected ray-pick + amber highlight.
//!
//! Clone-safe (`Rc<BimInner>`) so the same adapter handle can be:
//!   - registered with `KamiApp::with_pipeline(adapter.clone())`
//!   - captured by a tick hook (`on_update`) to call
//!     `pick_from_camera_if_clicked` and `set_highlighted_by_id` each
//!     frame on click

use glam::{Mat4, Vec3};
use hecs::World;
use kami_app::{Camera, RenderPipeline};
use kami_bim::{ElementKind, SceneGeom, StoreyScene};
use kami_render::RenderContext;
use std::cell::{Cell, RefCell};
use std::rc::Rc;

use crate::scene_mesh::SceneMeshCore;

struct CpuPickData {
    element_id: String,
    kind: ElementKind,
    positions: Vec<[f32; 3]>,
    normals: Vec<[f32; 3]>,
    indices: Vec<u32>,
    base_color: [f32; 3],
}

struct BimInner {
    core: RefCell<SceneMeshCore>,
    cpu: RefCell<Vec<CpuPickData>>,
    selected: Cell<Option<usize>>,
    device: wgpu::Device,
}

#[derive(Clone)]
pub struct BimSceneAdapter {
    inner: Rc<BimInner>,
}

#[derive(Debug, Clone)]
pub struct Pick {
    pub element_id: String,
    pub kind: ElementKind,
    pub world_pos: Vec3,
    pub distance: f32,
}

impl BimSceneAdapter {
    pub fn new(ctx: &RenderContext) -> Self {
        Self {
            inner: Rc::new(BimInner {
                core: RefCell::new(SceneMeshCore::new(ctx, "bim_scene", 0.0008)),
                cpu: RefCell::new(Vec::new()),
                selected: Cell::new(None),
                device: ctx.device.clone(),
            }),
        }
    }

    pub fn from_storey_scene(ctx: &RenderContext, scene: &StoreyScene) -> Self {
        let s = Self::new(ctx);
        s.ingest_storey(ctx, scene);
        s
    }

    pub fn ingest_storey(&self, ctx: &RenderContext, scene: &StoreyScene) {
        let mut core = self.inner.core.borrow_mut();
        let mut cpu = self.inner.cpu.borrow_mut();
        for item in &scene.items {
            let world = daffine_to_mat4(&item.world_transform);
            if let SceneGeom::Triangles { positions, indices, normals } = &item.geom {
                let world_positions: Vec<[f32; 3]> = positions
                    .iter()
                    .map(|p| {
                        let wp = world.transform_point3(Vec3::new(p[0], p[1], p[2]));
                        [wp.x, wp.y, wp.z]
                    })
                    .collect();
                let normal_mat = world.inverse().transpose();
                let world_normals: Vec<[f32; 3]> = normals
                    .iter()
                    .map(|n| {
                        let wn = normal_mat.transform_vector3(Vec3::new(n[0], n[1], n[2])).normalize_or_zero();
                        [wn.x, wn.y, wn.z]
                    })
                    .collect();
                core.push_batch(ctx, positions, normals, indices, item.base_color, world);
                cpu.push(CpuPickData {
                    element_id: bim_id_string(&item.element_id),
                    kind: item.kind,
                    positions: world_positions,
                    normals: world_normals,
                    indices: indices.clone(),
                    base_color: item.base_color,
                });
            }
        }
    }

    pub fn batch_count(&self) -> usize {
        self.inner.core.borrow().batches.len()
    }

    pub fn pick_ray(&self, origin: Vec3, dir: Vec3) -> Option<Pick> {
        let dir = dir.normalize_or_zero();
        if dir.length_squared() < 1.0e-6 {
            return None;
        }
        let cpu = self.inner.cpu.borrow();
        let mut best: Option<(f32, usize, Vec3)> = None;
        for (i, batch) in cpu.iter().enumerate() {
            for tri in batch.indices.chunks_exact(3) {
                let a = Vec3::from_array(batch.positions[tri[0] as usize]);
                let b = Vec3::from_array(batch.positions[tri[1] as usize]);
                let c = Vec3::from_array(batch.positions[tri[2] as usize]);
                if let Some(t) = ray_triangle(origin, dir, a, b, c) {
                    match best {
                        Some((best_t, _, _)) if best_t <= t => {}
                        _ => best = Some((t, i, origin + dir * t)),
                    }
                }
            }
        }
        best.map(|(t, i, p)| Pick {
            element_id: cpu[i].element_id.clone(),
            kind: cpu[i].kind,
            world_pos: p,
            distance: t,
        })
    }

    /// Mouse-projected pick from a camera. Uses
    /// `Camera::ray_from_ndc(mouse_ndc_x, mouse_ndc_y)` so the click
    /// targets the element under the cursor.
    pub fn pick_from_camera_if_clicked(&self, camera: &mut Camera) -> Option<Pick> {
        if !camera.consume_action() {
            return None;
        }
        let (origin, dir) = camera.ray_from_ndc(camera.mouse_ndc_x, camera.mouse_ndc_y);
        self.pick_ray(origin, dir)
    }

    pub fn set_highlighted_by_id(&self, element_id: &str) {
        let idx = {
            let cpu = self.inner.cpu.borrow();
            cpu.iter().position(|b| b.element_id == element_id)
        };
        self.set_highlighted(idx);
    }

    pub fn set_highlighted(&self, batch_index: Option<usize>) {
        if self.inner.selected.get() == batch_index {
            return;
        }
        if let Some(prev) = self.inner.selected.get() {
            self.rebuild_batch_color(prev, None);
        }
        if let Some(next) = batch_index {
            self.rebuild_batch_color(next, Some([1.0, 0.78, 0.22]));
        }
        self.inner.selected.set(batch_index);
    }

    pub fn selected_element(&self) -> Option<(String, ElementKind)> {
        let cpu = self.inner.cpu.borrow();
        self.inner
            .selected
            .get()
            .and_then(|i| cpu.get(i).map(|b| (b.element_id.clone(), b.kind)))
    }

    fn rebuild_batch_color(&self, idx: usize, override_color: Option<[f32; 3]>) {
        let cpu = self.inner.cpu.borrow();
        let Some(batch) = cpu.get(idx) else { return };
        let color = override_color.unwrap_or(batch.base_color);
        let mut core = self.inner.core.borrow_mut();
        core.replace_batch_color(&self.inner.device, idx, &batch.positions, &batch.normals, &batch.indices, color);
    }
}

impl RenderPipeline for BimSceneAdapter {
    fn record(
        &self,
        ctx: &RenderContext,
        encoder: &mut wgpu::CommandEncoder,
        view: &wgpu::TextureView,
        depth_view: &wgpu::TextureView,
        camera: &Camera,
        _world: &World,
    ) {
        let core = self.inner.core.borrow();
        core.record(ctx, encoder, view, depth_view, camera);
    }
}

fn daffine_to_mat4(a: &glam::DAffine3) -> Mat4 {
    let x = a.matrix3.x_axis;
    let y = a.matrix3.y_axis;
    let z = a.matrix3.z_axis;
    let t = a.translation;
    Mat4::from_cols(
        glam::Vec4::new(x.x as f32, x.y as f32, x.z as f32, 0.0),
        glam::Vec4::new(y.x as f32, y.y as f32, y.z as f32, 0.0),
        glam::Vec4::new(z.x as f32, z.y as f32, z.z as f32, 0.0),
        glam::Vec4::new(t.x as f32, t.y as f32, t.z as f32, 1.0),
    )
}

fn bim_id_string(id: &kami_bim::BimId) -> String {
    match id {
        kami_bim::BimId::Guid(s) => s.clone(),
        kami_bim::BimId::Local(n) => format!("local:{n}"),
    }
}

fn ray_triangle(origin: Vec3, dir: Vec3, v0: Vec3, v1: Vec3, v2: Vec3) -> Option<f32> {
    const EPS: f32 = 1.0e-6;
    let edge1 = v1 - v0;
    let edge2 = v2 - v0;
    let h = dir.cross(edge2);
    let a = edge1.dot(h);
    if a.abs() < EPS {
        return None;
    }
    let f = 1.0 / a;
    let s = origin - v0;
    let u = f * s.dot(h);
    if !(0.0..=1.0).contains(&u) {
        return None;
    }
    let q = s.cross(edge1);
    let v = f * dir.dot(q);
    if v < 0.0 || u + v > 1.0 {
        return None;
    }
    let t = f * edge2.dot(q);
    if t > EPS { Some(t) } else { None }
}
