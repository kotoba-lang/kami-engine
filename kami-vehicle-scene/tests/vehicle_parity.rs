...
use kami_vehicle::{VehicleKind, build_vehicle};
use kami_vehicle_scene::{ALL_VEHICLE_KINDS, build_from_edn};
...
fn vehicle_edn_build_matches_build_vehicle() {
    for kind in ALL_VEHICLE_KINDS {
        ...
        assert_eq!(edn.nodes.len(), oracle.nodes.len(), "{id}: node count");
        assert_eq!(edn.wheels.len(), oracle.wheels.len(), "{id}: wheel count");
        ...
        assert_eq!(
            edn.powertrain.engine.max_rpm, oracle.powertrain.engine.max_rpm,
            "{id}: engine max_rpm"
        );
        ...
        assert_eq!(
            edn.powertrain.gearbox.final_drive, oracle.powertrain.gearbox.final_drive,
        ...
        for (w, (we, wo)) in edn.wheels.iter().zip(oracle.wheels.iter()).enumerate() {
            assert_eq!(
                we.tire.d_long, wo.tire.d_long,
                "{id}: wheel[{w}] tire d_long"
            );
...
fn sports_is_sticky_others_are_road_dry() {
    ...
    assert_eq!(
        sports.wheels[0].tire.d_long, oracle_sports.wheels[0].tire.d_long,
    ...
    for kind in [VehicleKind::Sedan, VehicleKind::Suv, VehicleKind::Bus] {
        ...
        assert_eq!(
            ...
            o.wheels[0].tire.d_long,
            "{}: non-sports d_long == oracle",
            kind.id()
        );
        assert_eq!(
            v.wheels[0].tire.d_lat,
            o.wheels[0].tire.d_lat,
