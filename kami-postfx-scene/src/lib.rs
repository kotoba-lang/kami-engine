    ...
    pub fn from_post_effect(e: &PostEffect) -> Self {
        match *e {
            ...
            PostEffect::Vignette { intensity, radius } => {
                EffectSpec::Vignette { intensity, radius }
            }
            PostEffect::CRT {
                scanline_intensity,
                curvature,
...
fn pipeline_from_vec(effects: &[EdnValue]) -> Result<PostFxPipeline, Error> {
    ...
    for v in effects {
        let m = v
            .as_map()
            .ok_or_else(|| Error::UnknownEffect("<not-a-map>".to_string()))?;
...
mod tests {
    ...
    fn preset_lengths_match_builtin() {
        ...
        assert_eq!(
            shipped_preset("baminiku-character").unwrap().effects.len(),
            6
        );
    }
