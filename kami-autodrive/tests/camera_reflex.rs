...
use glam::{Vec2, Vec3};
use kami_autodrive::{Autopilot, AutopilotConfig, DriveState, Pose2, VehicleClass};
...
fn camera_only_reflex_brakes_for_a_close_wall() {
    ...
    assert!(
        cmd.brake > 0.5,
        "camera reflex should emergency-brake (brake={})",
        cmd.brake
    );
    assert!(
        cmd.throttle == 0.0,
        "should not be accelerating into the wall"
    );
...
fn no_phantom_brake_without_an_obstacle() {
    ...
    assert!(
        cmd.brake < 0.5,
        "open road must not emergency-brake (brake={})",
        cmd.brake
    );
}
