// Palette-driven terrain shader (4 materials × base+tip colors via splatmap weights).
struct U {
  viewProj: mat4x4f,
  ...
  baseCol: array<vec4f, 4>,
  tipCol: array<vec4f, 4>,
};
@group(0) @binding(0) var<uniform> u: U;

struct VIn {
  @location(0) pos: vec3f, @location(1) normal: vec3f,
  @location(2) uv: vec2f, @location(3) splat: vec4f,
};
struct VOut {
  @builtin(position) clipPos: vec4f,
  @location(0) wp: vec3f, @location(1) n: vec3f, @location(2) sp: vec4f,
};
