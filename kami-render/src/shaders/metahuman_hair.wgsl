// MetaHuman Hair Shader — Kajiya-Kay anisotropic BRDF + strand alpha.
//
// Hair card rendering with:
//   - Kajiya-Kay diffuse (tangent-based)
//   - Dual specular lobes (R: cuticle reflection, TRT: colored secondary)
//   - Root-to-tip color gradient + alpha fade
//   - Anisotropic highlight shifts
...
struct HairParams {
    ...
    shift_primary: f32,
    shift_secondary: f32,
    specular_power_r: f32,
    specular_power_trt: f32,
    specular_strength_r: f32,
    specular_strength_trt: f32,
    transmission: f32,
    ambient_occlusion_root: f32,
    _pad: f32,
};
...
fn vs_main(in: VertexInput) -> VertexOutput {
    ...
    // Tangent along hair strand direction (V axis = root to tip)
    let tangent = normalize(vec3<f32>(0.0, -1.0, 0.0)); // vertical strand direction
...
/// Shift tangent along normal for highlight control.
...
/// Anisotropic specular lobe: sin-based power.
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    if (alpha_sample < 0.05) { discard; }

    // Root-to-tip parameter
    let root_tip = in.uv.y;

    // Root darkening (ambient occlusion)
    let ao = mix(hair.ambient_occlusion_root, 1.0, sqrt(root_tip));

    // Color gradient: darker at root, base color at mid, lighter at tip
    ...
    // Primary specular (R lobe): shifted toward root, white highlight
    ...
    // Secondary specular (TRT lobe): shifted toward tip, colored
    ...
    // Transmission (TT lobe): back-lit glow
    ...
    let color = (ambient +
        (strand_color * diffuse * ao +
         vec3<f32>(1.0, 0.98, 0.95) * spec_R +
         hair.highlight_color * spec_TRT +
         tt_color) * light.color * light.intensity);
