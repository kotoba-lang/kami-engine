// Billboard particle — camera-facing quad with fade-by-age.
// Instance attributes: position(3), color(3), size(1), age(1), life(1).
...
fn vs(i: VsIn) -> VsOut {
    let world = i.ipos
        + u.cam_right * (i.quad.x * i.isize)
        + u.cam_up    * (i.quad.y * i.isize);
    var o: VsOut;
...
fn fs(i: VsOut) -> @location(0) vec4<f32> {
    // Soft circle cutoff (distance from quad center) for a sparkle look.
    let d = length(i.uv) * 2.0;  // 0 at center, ~1.4 at corner
