// KAMI Engine — Skinned MToon shader (VRM toon + GPU skinning)
//
// Identical to mtoon.wgsl except the vertex stage blends a bone palette.
// Bind group layout: 0 camera/light, 1 material+textures, 2 shadow, 3 bone palette.
...
// Bone palette: world * inverse_bind matrices, one per joint.
@group(3) @binding(0) var<storage, read> bones: array<mat4x4<f32>>;

// Morph target deltas + weights. Per-batch storage, per-batch uniform.
struct MorphInfo {
    target_count: u32,
    vertex_count: u32,
    _pad0: u32,
    _pad1: u32,
    weights: array<vec4<f32>, 16>, // up to 64 morph weights
};
