// KAMI Engine — MToon shader (VRM toon shading)
//
// Compatible with VRM MToon material: 2-color step shading + rim light.
// Shares bind group layout with pbr.wgsl (same uniforms, different fragment logic).
//
// MToon mode activated by: sss_model == 99 (magic value in MaterialUniform)

// ── Uniforms (same as PBR) ──
...
struct MaterialUniform {
    ...
    has_albedo_tex: u32,
    has_normal_tex: u32,
    subsurface_color: vec4<f32>,  // MToon: shade_color (RGB) + shade_shift (A)
    sss_r0: f32,                  // MToon: shade_toony
    sss_r1: f32,                  // MToon: rim_intensity
    sss_r2: f32,                  // MToon: rim_fresnel_power
    sss_model: u32,               // 99 = MToon mode
    aniso_t0: f32,                // MToon: outline_width
    aniso_t1: f32,
    aniso_t2: f32,
    aniso_strength: f32,
    hair_scatter: vec4<f32>,      // MToon: rim_color (RGB) + rim_lift (A)
    clearcoat: f32,
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // Shade color (from subsurface_color, repurposed for MToon)
    // shade_color.rgb is the tinted shade multiplier, shade_shift (A) controls threshold
    let shade_color = material.subsurface_color.rgb; // Already includes tint (base*0.85)
    let shade_shift = material.subsurface_color.a;   // Negative = more lit area
    let shade_toony = material.sss_r0;               // 0=smooth, 1=hard step

    // NdotL → step shading (VRM MToon standard)
    let ndl = dot(n, l);
    let shade_width = max(0.01, 1.0 - shade_toony);
    let shade_t = smoothstep(shade_shift - shade_width, shade_shift + shade_width, ndl);

    // Mix shade and lit: shade is darker tint of albedo, lit is full albedo
    ...
    // Rim light (parametric rim — Fresnel-based)
    ...
    // Ambient (hemisphere) — brighter for toon style
    ...
    // Gamma correction (no tone mapping for toon — preserve flat colors)
