...
struct SkinParams {
    ...
    hemoglobin_density: f32,
    subdermal_scatter: f32,
    pore_density: f32,
    oiliness: f32,
    _pad: f32,
};
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let epidermis_color = vec3<f32>(
        1.0 - skin.melanin_density * 0.6,
        1.0 - skin.melanin_density * 0.3,
        1.0 - skin.melanin_density * 0.1
    );
    ...
    let dermis_color = vec3<f32>(
        0.9 + skin.hemoglobin_density * 0.1,
        0.4 + skin.hemoglobin_density * 0.15,
        0.3
    );
    ...
    let diffuse_sss = base * n_dot_l * (1.0 - sss_factor) +
                      base * (sss_epid + sss_derm) * sss_factor;

    // --- Specular (GGX BRDF) ---
    let f0 = 0.028; // skin Fresnel reflectance at normal incidence
