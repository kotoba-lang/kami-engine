// KAMI Engine — 3D Gaussian Splatting shader (experimental)
//
// Two stages:
//   1. Compute: calculate distance from camera → sort keys
//   2. Render: billboard quad per splat, Gaussian alpha falloff
//
// Splats are rendered back-to-front (sorted far-to-near) with alpha blending
// after the PBR pass. Depth read (no write) ensures PBR geometry occludes splats.

// ── Splat data (storage buffer) ──

struct GaussianSplat {
    position: vec3<f32>,
    opacity: f32,           // log-space, sigmoid at render
    scale: vec3<f32>,       // log-space, exp at render
    _pad0: f32,
    rotation: vec4<f32>,    // quaternion wxyz
    sh_dc: vec3<f32>,       // SH DC band (color)
    _pad1: f32,
}
...
fn cs_compute_distances(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let dist = dot(diff, diff); // squared distance (sufficient for sorting)
    sort_entries[idx] = SortEntry(dist, idx);
}

// ── Render: billboard quad per splat ──
...
struct SplatVertexOutput {
    ...
    @location(2) quad_pos: vec2<f32>,    // [-1, 1] within billboard quad
    @location(3) conic: vec3<f32>,       // inverse 2D covariance (upper triangle: a, b, c)
}

/// Build rotation matrix from quaternion (wxyz).
...
fn vs_splat(
    ...
    let m = rot * s;
    let sigma = m * transpose(m); // 3D covariance
    ...
    // View rotation (upper 3x3 of view matrix)
    ...
    // Extract 2D covariance components + low-pass filter
    let a = cov2d[0][0] + 0.3;
    let b = cov2d[0][1];
    let c = cov2d[1][1] + 0.3;

    // Compute eigenvalues for billboard extent
    let det = a * c - b * b;
    ...
    out.color = max(splat.sh_dc + 0.5, vec3<f32>(0.0)); // SH DC to color
    out.alpha = 1.0 / (1.0 + exp(-splat.opacity));       // sigmoid
    out.quad_pos = corner * radius;
    out.conic = vec3<f32>(c * det_inv, -b * det_inv, a * det_inv); // inverse cov2d
    return out;
}
