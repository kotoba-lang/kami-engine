// Overcast sky dome + cloud ray-plane intersect (full-screen triangle, depth=1).
struct U {
  invVP: mat4x4f,
  camPos: vec3f, _p0: f32,
  sunDir: vec3f, _p1: f32,
  fogColor: vec3f, overcast: f32,
  scrollX: f32, scrollZ: f32, altitude: f32, _p2: f32,
};
...
@vertex fn vs(@builtin(vertex_index) vi: u32) -> VO {
  ...
  var o: VO; o.pos = vec4f(x, y, 1.0, 1.0); o.ndc = vec2f(x, y); return o;
}
...
fn fbm(p: vec2f) -> f32 {
  ...
  for (var i = 0; i < 5; i = i + 1) { v += a * n2(p * f); a *= 0.5; f *= 2.0; }
  return v;
}
...
@fragment fn fs(v: VO) -> @location(0) vec4f {
  ...
  let horizon = vec3f(0.72, 0.73, 0.75); let zenith = vec3f(0.60, 0.63, 0.67);
