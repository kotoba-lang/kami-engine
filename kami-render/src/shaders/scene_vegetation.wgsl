...
struct U {
  ...
  windDir: vec2f, gustMul: f32, biomeDry: f32,
};
@group(0) @binding(0) var<uniform> u: U;

struct VIn {
  @location(0) vpos: vec3f, @location(1) vuv: vec2f,
  @location(2) ipos: vec3f, @location(3) isc: f32,
  @location(4) irot: f32,  @location(5) isp: f32,
  @location(6) iph: f32,   @location(7) itn: f32,
};
struct VO {
  @builtin(position) cp: vec4f,
  @location(0) wp: vec3f, @location(1) uv: vec2f,
  @location(2) tn: f32, @location(3) sp: f32, @location(4) hf: f32,
};
...
fn wf_noise(p: vec2f) -> f32 {
  ...
  return mix(mix(wf_hash(i), wf_hash(i+vec2f(1,0)), s.x),
             mix(wf_hash(i+vec2f(0,1)), wf_hash(i+vec2f(1,1)), s.x), s.y);
}
...
fn wind_field_sample(xz: vec2f, t: f32, base_dir: vec2f, gust: f32) -> vec3f {
  let freq = 0.012;   // ~83m ripple wavelength
  let tfreq = 0.25;
  let var_amp = 0.5;
  let n1 = wf_noise(vec2f(xz.x * freq + t * tfreq, xz.y * freq + t * tfreq * 0.7)) * 2.0 - 1.0;
  let n2 = wf_noise(vec2f(xz.x * freq * 2.0 + t * tfreq * 1.5 + 13.0,
                          xz.y * freq * 2.0 + t * tfreq * 1.1 + 7.0)) * 2.0 - 1.0;
...
@vertex fn vs(v: VIn) -> VO {
  let h = sH(v.isp) * v.isc; let w = sW(v.isp) * v.isc;
  var lp = vec3f(v.vpos.x * w, v.vpos.y * h, v.vpos.z * w);
  let c = cos(v.irot); let s = sin(v.irot);
  ...
  o.wp = wp; o.uv = v.vuv; o.tn = v.itn; o.sp = v.isp; o.hf = hf;
  return o;
}
...
@fragment fn fs(v: VO) -> @location(0) vec4f {
  ...
  let gb = vec3f(0.18, 0.42, 0.08); let gt = vec3f(0.42, 0.68, 0.15);
  let db = vec3f(0.55, 0.48, 0.26); let dt = vec3f(0.78, 0.68, 0.38);
  let b = mix(gb, db, u.biomeDry); let t = mix(gt, dt, u.biomeDry);
  ...
  let ao = 1.0 - exp(-v.hf * 3.0);  // 0 at base, ~1 at top
  col *= mix(0.55, 1.0, ao);
